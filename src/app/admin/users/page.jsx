...
const [selectedUser, setSelectedUser] = useState(null);
  ...
  const filteredUsers = users.filter((user) => {
    const userName = user.name.toLowerCase(); // Cambio aquí
...
{selectedUser && (
  <div>
    ...
    {isEditing ? (
      <>
        <div className="mb-4">
          ...
          <input
            ...
            className={`border border-rosybrown rounded p-1 mb-2 ${isEditing ? 'bg-rosybrown-light' : ''}`}
          />
        </div>
      ...
      <>
        <p><span className="font-bold">Imagen:</span> {selectedUser.img && <Image src={selectedUser.img} alt={`${selectedUser.name} ${selectedUser.lastname}`} width={300} height={300} className="rounded-lg" />}</p>
        <p><span className="font-bold">Apellido:</span> {selectedUser.lastname}</p>
        <p><span className="font-bold">Nombre de usuario:</span> {selectedUser.username}</p>
        <p><span className="font-bold">Dirección:</span> {selectedUser.adress}</p>
        <p><span className="font-bold">Ciudad:</span> {selectedUser.city}</p>
        <p><span className="font-bold">Provincia:</span> {selectedUser.province}</p>
        <p><span className="font-bold">Rol:</span> {selectedUser.role}</p>
        <p><span className="font-bold">Email:</span> {selectedUser.email}</p>
        <p><span className="font-bold">Código Postal:</span> {selectedUser.codeP}</p>
        <p><span className="font-bold">Token:</span> {selectedUser.token}</p>
        <p><span className="font-bold">Activo:</span> {selectedUser.active ? 'Sí' : 'No'}</p>
      </>
    )}
  </div>
)}
