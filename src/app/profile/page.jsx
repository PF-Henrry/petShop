...
const Profile = () => {
  ...
  return (
    <>
      {profileData === null || !buys?.length || !sessionStatus ? (
        <Loading />
      ) : (
        <div className={`profile-container ${editable ? "editable" : ""}`}>
          {showSuccessMessage && (
            <div className="px-4 py-2 mt-4 text-center text-white bg-green-500 rounded">
              Cambios guardados exitosamente. Haga click en cancelar para salir
              del modo de edición.
              {goToTop()}
            </div>
          )}
          ...
          <div
            className={`flex flex-col gap-10 ${editable ? "hidden" : "block"}`}
          >
